...
// Classify.cpp : 응용 프로그램에 대한 클래스 동작을 정의합니다.
//
#include "stdafx.h"
...
void CClassifyApp::OnFileNew()
{
	...
	POSITION pos = GetFirstDocTemplatePosition(); //링크드리스트의 첫번째 위치 잡기

	for (int i = 0; i < m_poscntDocTemp; i++) //문서의 링크드리스트에 등록된 순번 만큼 이동처리루프.
		pDocTemplate = GetNextDocTemplate(pos);	//링크드리스트의 다음 포지션으로.
